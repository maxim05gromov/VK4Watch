...
class NewsInterfaceController: WKInterfaceController{
    @IBOutlet weak var WelcomeLabel1: WKInterfaceLabel!
    @IBOutlet weak var loadingLabel: WKInterfaceLabel!
    @IBOutlet weak var TableView: WKInterfaceTable!
    @IBOutlet weak var WelcomeLabel2: WKInterfaceLabel!
    ...
    override func awake(withContext context: Any?) {
        ...
        if UserDefaults.standard.string(forKey: "Token") != nil {
            if UserDefaults.standard.string(forKey: "Token") != "" {
                ...
                session.dataTask(with: url, completionHandler: { data, response, error in
                    ...
                    do{
                        ...
                        for item in 0...news.response.items.count - 1{
                            ...
                            if news.response.items[item].source_id! < 0{
                                ...
                                for name in 0...news.response.profiles.count - 1{
                                    if news.response.items[item].source_id! == news.response.profiles[name].id{
                                        controller.NameText.setText(news.response.profiles[name].first_name! + " " + news.response.profiles[name].last_name!)
                                        URLSession.shared.dataTask(with: URL(string: news.response.profiles[name].photo_50!)!) { data1, response1, error1 in
                                            var image = UIImage(data: data1!)
                                            image = imageWithRoundedCornerSize(cornerRadius: 17.5, usingImage: image!)
                ...
                WelcomeLabel1.setHidden(false)
                WelcomeLabel2.setHidden(false)
